    ...
    return json_response(
        {
            ...
            "application": APPLICATION_NAME,
            "version": APPLICATION_VERSION,
            "historical_rebuild_engine": _HISTORICAL_REBUILD_ENGINE_VERSION,
            "reset_engine": _RESET_ENGINE_VERSION,
...
def _run_full_reconciliation_dispatch(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        from engine.database import (
            ...
            replace_latest_sfda_snapshot,
            save_full_cancel_dispatch_pending_transactions,
            save_full_dispatch_pending_transactions,
            sync_batch_master_sfda_snapshot,
        )
        from engine.exporter import Exporter
        ...
        # Both directions inspect the same previous SFDA baseline. Cancel must
        # run first because the existing Dispatch confirmation owns the final
        # baseline replacement for backward compatibility.
        cancel_dispatch_confirmation = (
            confirm_full_cancel_dispatch_transactions_from_sfda(
                sfda_df,
                sfda_name,
            )
        ...
        cancel_pending_result = save_full_cancel_dispatch_pending_transactions(
            cancel_dispatch_upload.to_dict(orient="records"),
            run_number,
        )
        ...
        summary = {
            ...
            "cancel_dispatch_pending_saved": int(cancel_pending_result.get("saved", 0)),
            "cancel_dispatch_schema_available": bool(
