...
def batch_master_build(
    ...
    try:
        ...
        try:
            ...
            logger.exception(
                "Batch Master calculation failed."
            )
            return error_response(
                "Failed to build Batch Master.",
                500,
                str(exc),
            )
        ...
        try:
            ...
            if (
                not isinstance(database_result, dict)
                or database_result.get("status")
                not in {"success", "Success", "Completed"}
            ):
                message = (
                    database_result.get(
                        "message",
